...
class ZhiLianPlatform extends BasePlatform {
  constructor(platformConfig) {
    super(platformConfig);
    this.platformKey = 'zhilian';
  }
  ...
  async login(page, credentials) {
    try {
      ...
      // 选择手机号登录
      await this.safeClick(page, '.phone-tab');
  ...
  async searchJobs(page, jobFilters) {
    try {
      ...
      console.error('智联招聘搜索职位失败:', error);
      return {
        success: false,
  ...
  async extractJobListings(page) {
    try {
      ...
      const jobs = await page.evaluate(() => {
        ...
        jobCards.forEach(card => {
          try {
            ...
            const experienceElement = card.querySelector('.job-requirement');
      ...
      console.log(`成功提取${jobs.length}个智联招聘职位`);
      return jobs;
      
    } catch (error) {
      console.error('提取智联招聘职位列表失败:', error);
      return [];
    }
  }
  ...
  async submitApplication(page, job, resumeData) {
    try {
      console.log(`正在申请智联招聘职位: ${job.title} @ ${job.company}`);
      ...
      if (isSuccess) {
        console.log(`智联招聘职位申请成功: ${job.title}`);
        return {
          success: true,
          message: '申请成功'
        };
      ...
      console.error(`智联招聘职位申请失败: ${job.title}`, error);
      return {
        success: false,
  ...
  async getDeliveryStatus(userId, applicationData) {
    try {
      console.log('正在获取智联招聘投递状态...');
      
      // 这里需要实现从智联招聘的"我的投递"页面获取状态
      ...
      console.error('获取智联招聘投递状态失败:', error);
      return {
        success: false,
  ...
  async checkAntiCrawl(page) {
    try {
      ...
      console.error('检查智联招聘反爬虫失败:', error);
      return false;
    }
  }
  ...
  async handleCaptcha(page) {
    try {
      ...
      console.error('处理智联招聘验证码失败:', error);
      return {
        success: false,
...
module.exports = ZhiLianPlatform;